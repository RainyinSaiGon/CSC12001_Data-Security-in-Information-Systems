...
BEGIN
    FOR r IN staff_cursor LOOP
        BEGIN
            EXECUTE IMMEDIATE 'CREATE USER ' || q_ident(r.USERNAME) || ' IDENTIFIED BY ' || q_literal(r.USER_PASSWORD);
        EXCEPTION
            WHEN OTHERS THEN
                IF SQLCODE = -1920 THEN
                    EXECUTE IMMEDIATE 'ALTER USER ' || q_ident(r.USERNAME) || ' IDENTIFIED BY ' || q_literal(r.USER_PASSWORD) || ' ACCOUNT UNLOCK';
                ELSE
                    RAISE;
                END IF;
        END;
